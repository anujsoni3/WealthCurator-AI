      ...
      <div className="mx-auto flex w-full max-w-[1260px] flex-col gap-2 px-3 py-2.5 sm:px-4 lg:px-5">
        <div className="flex items-center justify-between">
          <div className="min-w-0">
            <button
              ...
              className="text-[11px] font-semibold uppercase tracking-[0.16em] text-primary-600 transition hover:text-primary-700 dark:text-blue-400 dark:hover:text-blue-300"
            >
              Wealth Curator
            </button>
          </div>
            ...
            <div className="relative w-full">
              ...
              {searchQuery && (
                <button
                  ...
                  className="absolute right-16 top-1/2 -translate-y-1/2 rounded px-1.5 py-0.5 text-xs text-slate-500 hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200"
                >
                  Clear
                </button>
              )}
        ...
        {searchQuery && !isSearchPending && (
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {searchMatchCount} section{searchMatchCount === 1 ? '' : 's'} matched
          </p>
        )}
      </div>
    </header>
